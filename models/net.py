...
from models.conv import BEANConv, BEANConv_short
...
class GraphBEAN_short(nn.Module):
    def __init__(
        self,
        in_channels: Union[int, Tuple[int, int, int]],
        # in_channels_v: Union[int, Tuple[int, int, int]],
    ...
    def create_encoder(self):
        ...
        for i in range(self.n_layers_encoder):
            ...
            if i == self.n_layers_encoder - 1:
                self.encoder_convs.append(
                    BEANConv_short(in_channels, out_channels, node_self_loop=False)
                )
            else:
                self.encoder_convs.append(
                    BEANConv_short(in_channels, out_channels, node_self_loop=True)
                )
    ...
    def create_feature_decoder(self):
        ...
        for i in range(self.n_layers_decoder):
            ...
            self.decoder_convs.append(BEANConv_short(in_channels, out_channels))
